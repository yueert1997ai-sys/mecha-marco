...
try{
  ...
  await evaluate(cdp,`(()=>{const game=__MECHA_MARCO__.game;game.run.exitOpen=false;game.facilities42.forEach((item)=>{item.dead=false;item.hp=item.maxHp;delete item.visualState44});game.vanguardIdentity44.blade=28;game.vanguardIdentity44.counterReady=true;game.vanguardIdentity44.counterSeconds=3;game.renderer.camera.x=0;game.renderer.camera.y=game.room.stage42.centerY-1.05;game.ui.updateHud(game);game.render();__MECHA_MARCO__.mech3dRenderer()?.render(game);return true})()`);
  await capture(cdp,'4.4A-og04-exit-closed-844x390.png');
  await evaluate(cdp,`(()=>{const game=__MECHA_MARCO__.game;game.run.exitOpen=true;game.render();__MECHA_MARCO__.mech3dRenderer()?.render(game);return true})()`);
  await capture(cdp,'4.4A-og04-exit-open-844x390.png');
  await evaluate(cdp,`(()=>{const game=__MECHA_MARCO__.game;game.run.exitOpen=false;game.facilities42.forEach((item)=>{item.dead=true;item.hp=0;item.visualState44='destroyed'});game.render();__MECHA_MARCO__.mech3dRenderer()?.render(game);return true})()`);
  await capture(cdp,'4.4A-og04-spatial-obstacles-844x390.png');
...
function assertAudit(audit){
  ...
  if(audit.width!==844||audit.height!==390||!audit.inside||!audit.separate||!audit.visible||spatial.obstacles?.length!==4||!spatial.stageLabel||!spatial.missionLabel||!spatial.gateLabel||!spatial.connector||!loop.deflected||loop.bladeAfterDeflect!==63||loop.counterType!=='vanguard-counter44'||loop.counterPierce<3||loop.bladeAfterCounter!==28)throw new Error(`OG-04 HUD/identity audit failed: ${JSON.stringify(audit)}`);
