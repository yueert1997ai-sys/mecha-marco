...
async def run():
    ...
    try:
        async with async_playwright() as p:
            ...
            results['rig']=await page.evaluate('''() => {
              ...
              const state=entry?.root?.userData?.loadout415;
              return {
                exists:Boolean(state),
                visible:Boolean(state?.rig?.visible),
                shoulder:Boolean(state?.shoulder?.visible),
                pods:Boolean(state?.pods?.visible),
                fins:Boolean(state?.fins?.visible),
                emitters:Boolean(state?.emitters?.visible),
                saber:Boolean(state?.saber?.visible),
                shield:Boolean(state?.shield?.visible),
                core:Boolean(state?.core?.visible),
                drones:Boolean(state?.drones?.visible),
                duo:Boolean(state?.duo?.visible),
                key:state?.lastKey||'',
              };
            }''')
            ...
            await page.evaluate('''() => {
              const game=globalThis.__MECHA_MARCO__.game;
              game.run.credits=200;
              game.showShop();
            }''')
            await page.wait_for_selector('.loadout-dock')
            await page.wait_for_timeout(250)
            results['shopBefore']=await page.evaluate('''() => {
              const panel=document.querySelector('.panel');
              const dock=document.querySelector('.loadout-dock');
              const cards=[...document.querySelectorAll('.shop-item')];
              return {
                dock:Boolean(dock),cards:cards.length,
            ...
            results['shopAfter']=await page.evaluate('''() => {
              const game=globalThis.__MECHA_MARCO__.game;
              return {
                modules:game.run.modules.length,
                visual:game.player.visualLoadout,
                dockText:document.querySelector('.loadout-dock')?.innerText||'',
            ...
            results['compact']=await page.evaluate('''() => {
              const panel=document.querySelector('.panel');
              const dock=document.querySelector('.loadout-dock');
              const actions=document.querySelector('.shop-actions');
              return {
                panel:{x:panel.getBoundingClientRect().x,y:panel.getBoundingClientRect().y,w:panel.getBoundingClientRect().width,h:panel.getBoundingClientRect().height,scrollW:panel.scrollWidth,clientW:panel.clientWidth},
    ...
    if results['baseline']['visual']!='4.1.5-visual-loadout-pass' or results['baseline']['uiStyle']!='low-saturation-glass':
        raise RuntimeError(f"Wrong runtime: {results['baseline']}")
    visual=results['fullLoadout']['visual']
    if visual['moduleCount']<7 or visual['beamTier']<1 or visual['ordnanceTier']<2 or visual['droneBits']<1:
        raise RuntimeError(f'Loadout profile incomplete: {visual}')
    rig=results['rig']
    required=['exists','visible','shoulder','pods','fins','emitters','saber','shield','core','drones','duo']
    if any(not rig[key] for key in required):
        raise RuntimeError(f'Visual rig missing active layers: {rig}')
    before=results['shopBefore']
    if not before['dock'] or before['cards']<1 or before['scrollWidth']>before['clientWidth']+2:
        raise RuntimeError(f'Shop layout invalid: {before}')
    after=results['shopAfter']
    if after['modules']<=results['fullLoadout']['visual']['moduleCount'] or '战斗模型同步升级' not in after['toast']:
        raise RuntimeError(f'Shop purchase did not update loadout: {after}')
    compact=results['compact']
    if compact['panel']['scrollW']>compact['panel']['clientW']+2:
        raise RuntimeError(f'Compact shop overflow: {compact}')
