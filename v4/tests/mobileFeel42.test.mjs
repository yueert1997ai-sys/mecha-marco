...
test('mobile feel patch improves stick response, boost fire and arena entry',async()=>{
  ...
  const depth=await read('src/ui/depthUI416.js');
  ...
  assert.match(depth,/aimSensitivity/);
  assert.match(depth,/moveSensitivity/);
  assert.match(depth,/aimDeadZone/);
...
test('top-down upper-body view remains stable while 4.2 adds continuous campaign',async()=>{
  ...
  assert.match(main,/4\.3\.1-consequence-progression/);
  assert.match(sw,/continuousCampaign42\.js/);
  assert.match(sw,/campaign42\.css/);
  assert.equal(version.trim(),'4.3.1-consequence-progression');
});
